...
func main() {
	...
	for i := 0; i < int(numDevices); i++ {
		...
		pids, count, err := dev.DeviceGetAccountingPids(bufferSize / 100)
		if err != nil {
		...
		utilizations, count, err := dev.DeviceGetProcessUtilization(100, 10 * time.Second)
		if err != nil {
			...
			for _, sample := range utilizations {
				...
				if err != nil {
					fmt.Printf("\tdev.SystemGetProcessName() error: %v\n", err)
				} else {
					fmt.Printf(", name: %s\n", name)
				}
			}
