...
func (d Device) DeviceGetProcessUtilization(processCount uint, since time.Duration) ([]*Utilization, uint, error) {
	...
	if errorString(r) != nil {
		return nil, uint(runningProcess), errorString(r)
	}

	utilizations := make([]*Utilization, runningProcess)
	for index, utilization := range cUtilizations[:runningProcess] {
		u := &Utilization{
	...
	return utilizations, uint(runningProcess), errorString(r)
}
